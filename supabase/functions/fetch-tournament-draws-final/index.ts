...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const days = body.days || 7; // Number of days to fetch

    console.log(`Fetching tournament draws for ${days} days starting from ${date}`);

    const allMatches: MatchData[] = [];
    const tournaments = new Set();

    // Fetch matches for multiple days
    for (let i = 0; i < days; i++) {
      ...
      // Import cache utility (only once)
      if (!globalThis.saveToCache) {
        try {
          const cacheModule = await import('../_shared/sportradar-cache.ts');
          globalThis.saveToCache = cacheModule.saveToCache;
        } catch {
          globalThis.saveToCache = async () => {};
        }
      }
      
      try {
        const scheduleEndpoint = `/schedules/${dateStr}/summaries.json`;
        const response = await fetch(
          `https://api.sportradar.com/tennis/trial/v3/en${scheduleEndpoint}?api_key=${apiKey}`
        );
        ...
        if (response.ok) {
          ...
          if (globalThis.saveToCache) {
            await globalThis.saveToCache(scheduleEndpoint, data);
          }
          ...
          if (data.summaries && Array.isArray(data.summaries)) {
            for (const summary of data.summaries) {
              if (summary.sport_event && summary.sport_event.sport_event_context) {
                ...
                // Track tournaments
                if (context.tournament?.id) {
                  tournaments.add(JSON.stringify({
                    ...
                    end_date: context.tournament.end_date,
                  }));
                }
          ...
          console.log(`No data for ${dateStr} (${response.status})`);
        }
    ...
    // Update tournaments table with new tournaments
    const tournamentArray = Array.from(tournaments).map(t => JSON.parse(t));
    for (const tournament of tournamentArray) {
      const { error } = await supabaseAdmin
        .from("tournaments")
        .upsert(
          {
      ...
      if (error) {
        console.error(`Error upserting tournament ${tournament.id}:`, error);
      }
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        date_range: {
          start: date,
          days: days,
          end: new Date(new Date(date).getTime() + (days - 1) * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        },
        tournaments_found: tournamentArray.length,
        matches_found: allMatches.length,
        tournaments: tournamentArray,
        matches_sample: allMatches.slice(0, 20), // Return first 20 matches as sample
        message: `Successfully fetched ${allMatches.length} matches from ${tournamentArray.length} tournaments`,
      }),
      {
