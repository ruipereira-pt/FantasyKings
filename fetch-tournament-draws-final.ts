...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      try {
        const response = await fetch(
          `https://api.sportradar.com/tennis/trial/v3/en/schedules/${dateStr}/summaries.json?api_key=${apiKey}`
        );
    ...
    const tournamentArray = Array.from(tournaments).map(t => JSON.parse(t as string));
